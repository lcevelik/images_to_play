...
D = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene"               # undistorted 21k learned seed
OUT = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene-mcmc/gaussian_splat.ply"
t0 = time.time()
def log(m, l="INFO"): print(f"[{int(time.time()-t0)}s][{l}] {m}", flush=True)

print("=== MCMC foreground for combine (4M cap, 30k, LPIPS, anti-flicker prune) ===", flush=True)
