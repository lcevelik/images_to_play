...
OUT=r"F:/Codebase/images_to_play/simple_splat/App/processing/learned-21k-softround/gaussian_splat.ply"
t0=time.time()
def log(m,l="INFO"): print(f"[{int(time.time()-t0)}s][{l}] {m}",flush=True)
print("=== MCMC 4M + SOFT layer + ANISOTROPY reg (round Gaussians, Postshot-like) ===",flush=True)
train_mcmc(D, total_steps=30000, cap_max=4_000_000, sh_degree=3, use_lpips=True,
           strategy_name='mcmc', export_opacity_min=0.005, aniso_reg=0.01, needle_max=4.0,
