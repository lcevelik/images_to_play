...
def run_draft(images_dir, job_dir, status_cb, log):
...
def run_production(images_dir, job_dir, status_cb, log):
...
def _write_status(job_dir, step, progress, stage, status='processing', ply_path=None):
    import json
...
if __name__ == '__main__':
    # Detached entry point: python pipeline/recipes.py <draft|production> <images_dir> <job_dir>
    ...
    try:
        ...
        out = runner(images_dir, job_dir, status, log)
        _write_status(job_dir, 'Processing complete!', 100, None, status='completed', ply_path=out)
    except Exception as e:
        import traceback
