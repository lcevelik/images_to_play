...
def extract_camera_poses(sparse_dir):
    ...
    poses.sort(key=lambda p: _natural_key(p['name']))
...
def export_camera_gltf(poses, output_path, fps=30):
    """Export camera animation as a GLTF file (built manually for precise control).
...
def _export_gltf_manual(poses, output_path, fps=30):
    ...
    # Pack as binary data: [times | translations | rotations] in one buffer
    ...
    full_buffer = time_bytes + trans_bytes + rot_bytes
...
def export_point_cloud_ply(sparse_dir, output_path, center=False):
