...
D = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene2-dense"      # gap-filled seed (clean building + dense asphalt)
OUT = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene2-dense-mcmc/gaussian_splat.ply"
t0 = time.time()
def log(m, l="INFO"): print(f"[{int(time.time()-t0)}s][{l}] {m}", flush=True)

print("=== MCMC on GAP-FILL seed (4M, 30k, LPIPS) — clean building + dense asphalt ===", flush=True)
