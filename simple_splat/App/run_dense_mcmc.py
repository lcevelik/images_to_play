...
D = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene-dense"            # depth-seeded (1.07M) seed
OUT = r"F:/Codebase/images_to_play/simple_splat/App/processing/scene-dense-mcmc/gaussian_splat.ply"
t0 = time.time()
def log(m, l="INFO"): print(f"[{int(time.time()-t0)}s][{l}] {m}", flush=True)

print("=== MCMC on DEPTH-SEEDED dense seed (4M cap, 30k, LPIPS) — same settings as champion ===", flush=True)
