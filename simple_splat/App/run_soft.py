...
D=r"F:/Codebase/images_to_play/simple_splat/App/processing/learned-21k"          # sky-masked seed
OUT=r"F:/Codebase/images_to_play/simple_splat/App/processing/learned-21k-soft/gaussian_splat.ply"
t0=time.time()
def log(m,l="INFO"): print(f"[{int(time.time()-t0)}s][{l}] {m}",flush=True)
print("=== MCMC 4M + SKY MASK + LIGHT prune (keep soft layer, match Postshot count) ===",flush=True)
train_mcmc(D, total_steps=30000, cap_max=4_000_000, sh_degree=3, use_lpips=True,
           strategy_name='mcmc', export_opacity_min=0.005, output_ply_path=OUT, progress_callback=log)
