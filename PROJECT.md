...
- [x] 2026-06-27 — **Production VERIFIED end-to-end through the UI** (job `2f3883c2`: learned-SfM → undistort → MCMC+Brush parallel → combine σ=1.7 → 624 MB `combined.ply`, ~3 hr, status `completed`). Draft was already verified; this closes the Production gap.
- [x] 2026-06-27 — **FBX matchmove export actually fixed & Blender-verified** (`pipeline/fbx_binary.py`; it never imported into a DCC before — Blender crashed). Four bugs: binary object names (`Name\x00\x01Class` not `Class::Name`), 100× scale (`UnitScaleFactor=100` = metres, 1:1 with splat), ~90° orientation (Z-up axes + FBX +X-aim `_CAM_LOOK_FLIP`), keyframe timing (`TimeMode`+`CustomFrameRate`). Verified: pos err 0.0000, look·=1.000, up·=1.000.
- [x] 2026-06-27 — **FBX matches the SOURCE VIDEO fps**: `extract_frames_from_video` persists `video_info.json` (source_fps + interval); `video_frame_timing()` feeds the export the true fps with keys on original source-frame numbers (frame-accurate to the plate; consecutive when interval=1).
